...
public class AISearchChatGLM {
    ...
    private static void writeKnowledgeByFile(VectorDBClient client) {
...
/*        for (String f:
     ...
     private static String searchKnowledge(String question, VectorDBClient client) {
        ...
        Collection collection = client.database(DB_NAME).describeCollection(COLLECTION_NAME);
         ...
         SearchByEmbeddingItemsParam searchByEmbeddingItemsParam = SearchByEmbeddingItemsParam.newBuilder()
                 ...
                 // 指定返回的 fields
                 .withOutputFields(Arrays.asList("type","fault_code","text"))
                 .build();
    ...
    private static void initKnowledge(VectorDBClient client) throws Exception {
        ...
        String llmResult = ChatGLM6B.ask("空调温度升高发出嗡嗡声", knowledge);
    ...
    private static String askOnce(String question, VectorDBClient client) {
        String knowledge = getKnowledge(question, client);
        return getLLMResult(question, knowledge);
    }

    private static void askList(List<String> question, VectorDBClient client) {
        ...
        for (String q : question) {
            ...
            result.add("---->CHATGLM-6B回答结果：");
            result.add(llmResult);
            result.add("==================================================================================");

        }
        writeListToTxtFile(result, "result/chatglm/result.txt");
    }

    public static void main(String[] args) throws Exception {
        VectorDBClient client = VDBClientFactory.createClient();
        String filePath = "question/question_class1.txt";
